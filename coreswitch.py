...
with junos_driver(hostname=host, username=user, password=junos_password) as dev:
    ...
    mac_data = {'mac':  [entry['mac'] for entry in mac_table if entry['interface'] ],
            'interface': [entry['interface'] for entry in mac_table if entry['interface'] ],
            'vlan': [entry['vlan'] for entry in mac_table if entry['interface'] ]
...
filename = 'core switch report' +' ' + str_time + '.xlsx'
file = str(filename)
